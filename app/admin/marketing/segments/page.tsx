    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-border/50 bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 dark:from-green-950/20 dark:via-emerald-950/20 dark:to-teal-950/20 p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button onClick={openCreate} className="gap-2 shrink-0 rounded-xl bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 hover:scale-[1.02] transition-all duration-300">
      ...
      {segments.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {segments.map((seg) => (
            <Card key={seg.id} className="rounded-2xl border border-border/50 hover:shadow-2xl hover:scale-[1.02] transition-all duration-300">
              ...
              <CardContent className="pt-0 space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button variant="outline" size="sm" className="gap-1 text-destructive hover:text-destructive" onClick={() => { openEdit(seg); setDeleteConfirm(seg.id) }}>
      ...
      <Sheet open={sheetOpen} onOpenChange={(open) => !open && closeSheet()}>
        <SheetContent side="right" className="overflow-y-auto w-full sm:max-w-lg">
          <SheetHeader>
            ...
            <SheetDescription>{editingId ? 'Name, Beschreibung und Regeln anpassen.' : 'Zielgruppe anlegen und Regeln definieren.'}</SheetDescription>
          </SheetHeader>
          <div className="grid gap-4 py-6 space-y-3">
            <div className="grid gap-2">
              <Label htmlFor="seg-name">Name *</Label>
              <Input id="seg-name" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="z. B. Stammkunden" />
            </div>
            <div className="grid gap-2">
            ...
            <div className="grid gap-2">
              ...
              <select value={form.rulesCombination} onChange={(e) => setForm((f) => ({ ...f, rulesCombination: e.target.value as 'AND' | 'OR' }))} className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                ...
                <Card key={idx} className="p-3 rounded-xl border-border/50">
                  <div className="grid grid-cols-[1fr,1fr,auto] gap-2 items-end">
                    <div className="grid gap-1">
                      ...
                      <select value={rule.attribute} onChange={(e) => updateRule(idx, { attribute: e.target.value })} className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                        {SEGMENT_ATTRIBUTES.map((a) => <option key={a.id} value={a.id}>{a.label}</option>)}
                      </select>
                    </div>
                    <div className="grid gap-1">
                      <span className="text-xs text-muted-foreground">Operator</span>
                      <select value={rule.operator} onChange={(e) => updateRule(idx, { operator: e.target.value as SegmentRule['operator'] })} className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                        {operators.map((o) => <option key={o.id} value={o.id}>{o.label}</option>)}
                      </select>
                    </div>
                    <Button type="button" variant="ghost" size="icon" className="shrink-0" onClick={() => removeRule(idx)} aria-label="Regel entfernen">
            ...
            {editingId && (
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {audienceLoading ? <span className="text-sm text-muted-foreground">Berechnung…</span> : audienceCount !== null ? <Badge variant="secondary" className="bg-muted text-muted-foreground">{audienceCount} Kunden</Badge> : <Button variant="outline" size="sm" onClick={() => fetchAudience(editingId)} disabled={audienceLoading}>Berechnen</Button>}
                </div>
                <div className="space-y-2 border-t pt-4">
                  ...
                  {previewLoading ? (
                    ...
                    <div className="rounded-lg border border-border/50 overflow-hidden">
                      {audiencePreview.usersWithRules.length === 0 ? <p className="p-3 text-sm text-muted-foreground">Keine Kunden im Segment (oder keine Regeln definiert).</p> : (
                        <div className="max-h-64 overflow-y-auto">
                          <table className="w-full text-sm">
                            <thead className="bg-muted/50 sticky top-0">
                              <tr><th className="text-left p-2 font-medium">Kunde</th><th className="text-left p-2 font-medium">Erfüllte Regeln</th></tr>
                            </thead>
                            <tbody>
                                ...
                                <tr key={u.id} className="border-t border-border/50">
                                  ...
                                  <td className="p-2 align-top">
                                    <ul className="list-disc list-inside text-muted-foreground space-y-0.5">
                                      {u.matchedRuleLabels.map((label, i) => <li key={i}>{label}</li>)}
                                    </ul>
                                  </td>
                                </tr>
                              ))}
                      ...
                      {audiencePreview.count > audiencePreview.usersWithRules.length && <p className="p-2 text-xs text-muted-foreground border-t border-border/50">Weitere {audiencePreview.count - audiencePreview.usersWithRules.length} Kunden im Segment.</p>}
                    </div>
                  ) : editingId && !previewLoading && !audiencePreview ? <Button variant="outline" size="sm" onClick={() => fetchAudiencePreview(editingId)}>Preview laden</Button> : null}
                </div>
              </div>
            )}
            ...
            {deleteConfirm === editingId && editingId && (
              <div className="rounded-lg border border-destructive/50 bg-destructive/5 p-3 space-y-2">
                ...
                <div className="flex gap-2">
                  <Button variant="destructive" size="sm" onClick={() => handleDelete(editingId)}>Löschen</Button>
